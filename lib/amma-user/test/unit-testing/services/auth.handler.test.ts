...
suite('Test Article Service', () => {
    ...
    test("test validate with valid user, role doesn't exist", (next) => {
        let stub = Sinon.stub(userModel, 'findByToken', (token, callback) => {
            return callback(null, {isActive: true});
        });
        let roleStub = Sinon.stub(roleModel, 'findById', (role, callback) => {
    ...
    test('test validate with valid user, role error', (next) => {
        let stub = Sinon.stub(userModel, 'findByToken', (token, callback) => {
            return callback(null, {isActive: true});
        });
        let roleStub = Sinon.stub(roleModel, 'findById', (role, callback) => {
    ...
    test('Login - error', (next) => {
        let stub = Sinon.stub(userModel, 'canLogin', (id, password, callback) => {
            return callback('error', null);
        });
        let callback = Sinon.spy((err) => {
    ...
    test('Login - user not found', (next) => {
        let stub = Sinon.stub(userModel, 'canLogin', (id, password, callback) => {
...
*/
    test('Login - Success', (next) => {
        let stub = Sinon.stub(userModel, 'canLogin', (id, password, callback) => {
            return callback(null, {isActive: true});
        });
        let tokenStub = Sinon.stub(userModel, 'findByIdAndUpdateToken', (id, token, callback) => {
            return callback(null, {});
        });
        let callback = Sinon.spy((err, result) => {
            expect(err).to.be.not.exist();
    ...
    test('Logout - Success', (next) => {
        let stub = Sinon.stub(userModel, 'findByTokenAndRemove', (id, callback) => {
            return callback(null, {});
        });
        let callback = Sinon.spy((err) => {
            expect(err).to.be.not.exist();
