...
var UserProfileController = (function () {
    ...
    UserProfileController.prototype.createToken = function (request, reply) {
        var service = this.getService();
        var id = request.params.id;
        service.createToken(id, reply);
    };
    UserProfileController.prototype.getImagesUsingToken = function (request, reply) {
        var service = this.getService();
        var token = request.params.token;
        var files = service.getFilesByToken(token);
        reply({
            files: files
        });
    };
