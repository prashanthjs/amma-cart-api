...
var AuthHandler = (function () {
    ...
    AuthHandler.prototype.createAndAddToken = function (id, next) {
        ...
        userModel.findByIdAndUpdateToken(id, token, function (err) {
            ...
            return next(null, { token: JWT.sign({ token: token }, _this._getSecret()) });
        });
    };
        ...
        if (!token) {
            ...
            userModel.findByToken(token, function (err, result) {
                if (err) {
                    ...
                    roleModel.findById(result.role, function (err, res) {
                        if (!err) {
    ...
    AuthHandler.prototype.login = function (id, password, callback) {
        var _this = this;
        this._getUserModel().canLogin(id, password, function (err, user) {
            if (err) {
                callback(err);
            }
            else if (user) {
    ...
    AuthHandler.prototype.logout = function (token, callback) {
